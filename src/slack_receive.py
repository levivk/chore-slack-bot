...
def handle_im_picture(url: str, say: Say) -> None:
    ...
    kat_list = [ka for ka in kat]
    kat_list.sort(key=lambda i: i.date)
    kitchen_complete_names = [kat_list[i].id for i in left_buttons]
    chore_complete_names = [kat_list[i].id for i in right_buttons]
